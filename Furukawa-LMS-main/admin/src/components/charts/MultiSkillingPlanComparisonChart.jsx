...
import { useGetMultiSkillingPlanStatusQuery } from '@/Redux/AllApi/AdminHomeApi';
...
import { IconUsersGroup, IconCalendar, IconRefresh, IconChevronDown } from "@tabler/icons-react";
...
const MultiSkillingPlanComparisonChart = ({ departments: departmentsProp } = {}) => {
    ...
    const { data, isLoading, error } = useGetMultiSkillingPlanStatusQuery({
        groupBy: timeframe,
        startDate,
        endDate,
    ...
    const chartOptions = useMemo(() => ({
        ...
        tooltip: {
            ...
            formatter() {
                ...
                return (
                    ...
                    `<b>${title}</b> — ${this.point.isExpected ? t('charts.plannedMultiSkilling') : t('charts.actualMultiSkilling')}<br/>` +
        ...
        series: [
            {
                type: 'column',
                name: t('charts.plannedMultiSkilling'),
                data: expectedPoints,
                color: '#8b5cf6',
            },
            {
                type: 'column',
                name: t('charts.actualMultiSkilling'),
                data: actualPoints,
                color: '#3b82f6',
            }
        ],
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    <div className="space-y-1">
                        <CardTitle className="flex items-center gap-2 text-lg">
                            ...
                            {t('charts.multiSkillingPlanComparison')}
                        </CardTitle>
                        <CardDescription>
                            {t('charts.multiSkillingPlanComparisonDesc')}
                        </CardDescription>
                    </div>
                </div>
            ...
            <CardContent>
                {isLoading ? (
                    ...
                    <div style={{ height: isTablet ? 500 : isMobile ? 480 : 560 }} className="flex flex-col items-center justify-center text-red-500 gap-2">
                        <p className="text-sm font-semibold">{t('charts.failedToLoadMultiSkilling')}</p>
                    </div>
                ) : !hasAnyData ? (
                    <div style={{ height: isTablet ? 500 : isMobile ? 480 : 560 }} className="flex flex-col items-center justify-center text-gray-400 bg-gray-50/50 rounded-xl border border-dashed gap-2">
                        ...
                        <p className="text-sm font-medium">{t('charts.noMultiSkillingData')}</p>
...
export default MultiSkillingPlanComparisonChart;
