...
import { useGetSkillMatrixCertificateStatusQuery } from '@/Redux/AllApi/AdminHomeApi';
...
import { IconCertificate, IconCalendar, IconRefresh, IconChevronDown } from "@tabler/icons-react";
...
const SkillMatrixCertificateComparisonChart = ({ departments: departmentsProp } = {}) => {
    ...
    const { data, isLoading, error } = useGetSkillMatrixCertificateStatusQuery({
        groupBy: timeframe,
        startDate,
        endDate,
    ...
    const totalFilledItems = useMemo(
        () => Object.values(seriesTotals).reduce((a, b) => a + b, 0),
        [seriesTotals]
    );
    const hasAnyData = totalFilledItems > 0;
    ...
    const chartOptions = useMemo(() => ({
        ...
        yAxis: {
            min: 0,
            allowDecimals: false,
            title: { text: t('charts.filledItems'), style: { color: '#94a3b8', fontSize: '14px', fontWeight: 'bold' } },
        ...
        tooltip: {
            ...
            formatter() {
                ...
                return `<b style="font-size:14px;color:#0f172a">${deptName || ''}</b><br/>${dateLabel || ''}<br/><span style="color:${this.point.color}">●</span> ${t('charts.filledItems')}: <b>${this.y}</b>`;
            },
        },
        plotOptions: {
            column: {
            ...
            <CardHeader className="pb-4">
                <div className="space-y-1">
                    <CardTitle className="flex items-center gap-2 text-lg">
                        ...
                        {t('charts.skillMatrixComparison')}
                    </CardTitle>
                    <CardDescription>
                        {t('charts.skillMatrixComparisonDesc')}
                    </CardDescription>
                </div>
            ...
            <CardContent>
                {isLoading ? (
                    ...
                    <div style={{ height: emptyStateHeight }} className="flex flex-col items-center justify-center text-red-500 gap-2">
                        <p className="text-sm font-semibold">{t('charts.failedToLoadSkillMatrix')}</p>
                    </div>
                ) : !hasAnyData ? (
                    <div style={{ height: emptyStateHeight }} className="flex flex-col items-center justify-center text-gray-400 bg-gray-50/50 rounded-xl border border-dashed gap-2">
                        ...
                        <p className="text-sm font-medium">{t('charts.noSkillMatrixData')}</p>
                    ...
                    <>
                        ...
                        <div className="mt-5">
                            <div className="flex items-center justify-between p-2.5 rounded-lg bg-blue-50 mb-3">
                                <span className="text-xs font-bold text-blue-700">{t('charts.filledItems')}</span>
                                <div className="flex items-center gap-3">
                                    <span className="text-sm font-black text-blue-900">{totalFilledItems}</span>
                                    <Button
                                        variant="ghost"
                                        size="sm"
...
export default SkillMatrixCertificateComparisonChart;
