...
import { useGetSkillUpgradationPlanStatusQuery } from '@/Redux/AllApi/AdminHomeApi';
...
import { IconTrendingUp, IconCalendar, IconRefresh, IconChevronDown } from "@tabler/icons-react";
...
const SkillUpgradationPlanComparisonChart = ({ departments: departmentsProp } = {}) => {
    ...
    const { data, isLoading, error } = useGetSkillUpgradationPlanStatusQuery({
        groupBy: timeframe,
        startDate,
        endDate,
    ...
    // instead of showing one slot per department.
    ...
    const chartOptions = useMemo(() => ({
        ...
        tooltip: {
            ...
            formatter() {
                ...
                return (
                    ...
                    `<b>${title}</b> — ${this.point.isExpected ? t('charts.plannedUpgradation') : t('charts.actualUpgradation')}<br/>` +
        ...
        series: [
            {
                type: 'column',
                name: t('charts.plannedUpgradation'),
                data: expectedPoints,
                color: '#8b5cf6',
            },
            {
                type: 'column',
                name: t('charts.actualUpgradation'),
                data: actualPoints,
                color: '#3b82f6',
            }
        ],
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    <div className="space-y-1">
                        <CardTitle className="flex items-center gap-2 text-lg">
                            ...
                            {t('charts.skillUpgradationPlanComparison')}
                        </CardTitle>
                        <CardDescription>
                            {t('charts.skillUpgradationPlanComparisonDesc')}
                        </CardDescription>
                    </div>
                </div>
            ...
            <CardContent>
                {isLoading ? (
                    ...
                    <div style={{ height: isTablet ? 500 : isMobile ? 480 : 560 }} className="flex flex-col items-center justify-center text-red-500 gap-2">
                        <p className="text-sm font-semibold">{t('charts.failedToLoadSkillUpgradation')}</p>
                    </div>
                ) : !hasAnyData ? (
                    <div style={{ height: isTablet ? 500 : isMobile ? 480 : 560 }} className="flex flex-col items-center justify-center text-gray-400 bg-gray-50/50 rounded-xl border border-dashed gap-2">
                        ...
                        <p className="text-sm font-medium">{t('charts.noSkillUpgradationData')}</p>
...
export default SkillUpgradationPlanComparisonChart;
