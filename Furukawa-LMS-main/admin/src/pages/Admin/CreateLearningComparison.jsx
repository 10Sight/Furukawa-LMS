...
const CreateLearningComparison = () => {
    ...
    const FileUploadInput = ({ label, field, icon: Icon, color, accept }) => {
        ...
        return (
            <div className="space-y-2">
                ...
                {/* Staged files for this type */}
                {staged.map((file, i) => (
                    <div key={i} className="flex items-center justify-between p-2.5 rounded-lg border border-emerald-200 bg-emerald-50/50">
    ...
    const SidePanel = ({ prefix, selectedType, setSelectedType, borderColor, headerBg, dividerColor, accentBorder, placeholderDesc, descLabel }) => {
        const totalFiles = fileTypes.reduce((sum, t) => sum + files[`${prefix}${t.id}`].length, 0);
        return (
            <Card className={`border-none shadow-sm overflow-hidden border-t-4 ${borderColor}`}>
                <CardHeader className={headerBg}>
                    <CardTitle className="text-lg flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-lg ${prefix === 'before' ? 'bg-amber-100' : 'bg-emerald-100'} flex items-center justify-center`}>
                            {prefix === 'before'
                                ? <ArrowLeft className={`w-4 h-4 ${prefix === 'before' ? 'text-amber-600' : 'text-emerald-600'}`} />
                ...
                <CardContent className="p-6 space-y-5">
                    ...
                    <div className="space-y-4">
                        ...
                        {fileTypes.some(t => files[`${prefix}${t.id}`].length > 0) && (
                                    ...
                                    {fileTypes.flatMap(type =>
                                        files[`${prefix}${type.id}`].map((file, i) => (
                                            <div key={`${type.id}-${i}`} className={`flex items-center justify-between p-2 rounded-lg bg-white border ${prefix === 'before' ? 'border-amber-100' : 'border-emerald-100'}`}>
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => removeFile(`${prefix}${type.id}`, i)}
