...
const DPRManage = () => {
    ...
    const [remark, setRemark] = useState("Updated global manpower template");
    ...
    // Always fetch GLOBAL config for setup
    const { data: currentConfigResp } = useGetDPRConfigQuery("GLOBAL");
    ...
    useEffect(() => {
        if (currentConfigResp?.data?.config) {
            const config = currentConfigResp.data.config;
    ...
    const handleSaveSetup = async () => {
        ...
        try {
            const currentConfig = currentConfigResp?.data?.config || {};
            ...
            toast.success("Global DPR Manpower template saved!");
        } catch (error) {
                    ...
                    <CardContent className="flex-1 p-0 overflow-hidden flex flex-col">
                        ...
                        <div className="p-4 bg-white border-t border-slate-100">
                            <Button 
                                ...
                                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold h-12 shadow-lg shadow-blue-100 group"
