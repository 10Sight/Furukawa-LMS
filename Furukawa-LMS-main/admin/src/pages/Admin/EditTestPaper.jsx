...
const EditTestPaper = () => {
  ...
  // Populate data when quiz is fetched
  useEffect(() => {
    if (quizResponse?.data) {
      ...
      setFormData({
        ...
        sectionId: Array.isArray(q.sectionId) ? q.sectionId.map(String) : [],
        lineId: Array.isArray(q.lineId) ? q.lineId.map(String) : [],
        ...
        questions: Array.isArray(q.questions) ? q.questions.map(qItem => ({
          ...
          type: qItem.type || "mcq",
          marks: qItem.marks ?? 1,
          image: qItem.image || null,
          options: Array.isArray(qItem.options) ? qItem.options.map(opt => ({
            text: opt.text || "",
            textSec: opt.textSec || "",
            isCorrect: !!opt.isCorrect,
            image: opt.image || null
          })) : [
            { text: "", textSec: "", isCorrect: false, image: null },
            { text: "", textSec: "", isCorrect: false, image: null }
          ],
          correctAnswer: qItem.correctAnswer || "",
          correctAnswerSec: qItem.correctAnswerSec || "",
          pairs: Array.isArray(qItem.pairs) ? qItem.pairs.map(p => ({
            leftText: p.leftText || "",
  ...
  const lineOptions = React.useMemo(() => {
    ...
    if (formData.sectionId.length > 0) {
      return allLines.filter(l => formData.sectionId.includes(String(l.sectionId))).map(l => ({ value: String(l.id), label: l.name }));
    }
    return [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(error);
      toast.error(error?.data?.message || "Failed to update test paper");
    }
  };
  ...
  return (
    <div className="space-y-6 p-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Edit Test Paper</h1>
          <p className="text-muted-foreground mt-1">
            Update standalone assessment details, hierarchy context, and multi-format questions.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="border-none shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Set the core information and targets for this standalone assessment.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-2">
              ...
              <Input
                ...
                placeholder="e.g., Final Mechanical Assembly Evaluation"
                required
              />
            </div>
                    ...
                    <SelectValue
                      placeholder={
                        formData.departmentId.length > 0
                          ? `${formData.departmentId.length} selected`
                  ...
                  <SelectContent>
                    {departmentOptions.map((opt) => (
                      <SelectItem key={opt.value} value={opt.value}>
                        {opt.label}{" "}
                        {formData.departmentId.includes(opt.value) ? "✓" : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                ...
                >
                    ...
                    <SelectValue
                      placeholder={
                        formData.sectionId.length > 0
                          ? `${formData.sectionId.length} selected`
                          : "Select sections"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {sectionOptions.map((opt) => (
                      <SelectItem key={opt.value} value={opt.value}>
                        {opt.label}{" "}
                        {formData.sectionId.includes(opt.value) ? "✓" : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label>Target Lines (Optional)</Label>
                <Select
                ...
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        formData.lineId.length > 0
                          ? `${formData.lineId.length} selected`
                          : "Select lines"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {lineOptions.map((opt) => (
                      <SelectItem key={opt.value} value={opt.value}>
                        {opt.label}{" "}
                        {formData.lineId.includes(opt.value) ? "✓" : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <div className="flex flex-wrap gap-1 mt-1">
                  {formData.lineId.map((id) => (
                    <Badge key={id} variant="secondary" className="gap-1 bg-amber-50 text-amber-700 hover:bg-amber-100">
                      {lineOptions.find((o) => o.value === id)?.label || id}
                      <IconX
                        className="h-3 w-3 cursor-pointer"
              ...
              <div className="grid gap-2">
                <Label>Target Sub-Sections (Optional)</Label>
                <Select
                ...
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        formData.subSectionId.length > 0
                          ? `${formData.subSectionId.length} selected`
                          : "Select sub-sections"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {subSectionOptions.map((opt) => (
                      <SelectItem key={opt.value} value={opt.value}>
                        {opt.label}{" "}
                        {formData.subSectionId.includes(opt.value) ? "✓" : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <div className="flex flex-wrap gap-1 mt-1">
                  {formData.subSectionId.map((id) => (
                    <Badge key={id} variant="secondary" className="gap-1 bg-teal-50 text-teal-700 hover:bg-teal-100">
                      {subSectionOptions.find((o) => o.value === id)?.label || id}
                      <IconX
                        className="h-3 w-3 cursor-pointer"
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button type="submit" disabled={isSaving} className="gap-2 bg-blue-600 hover:bg-blue-700 text-white">
            ...
            {isSaving ? "Saving..." : "Save Test Paper"}
          </Button>
        </div>
      </form>
    </div>
