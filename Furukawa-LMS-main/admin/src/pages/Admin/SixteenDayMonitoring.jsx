...
const SixteenDayMonitoring = () => {
    ...
    const { isEmployee, hasManagePermission, canManageFeedback, canViewFeedback } = useMemo(() => {
        ...
        // If they have access to the page, they should at least be able to VIEW the feedback sheet
    ...
    // Fetch students based on available hierarchy
    const { data: studentsData, isFetching: isFetchingStudents } = useGetAllStudentsQuery({
        ...
        // Skip only if no meaningful filter is applied AND user isn't an employee
    ...
    const canEditConfig = useMemo(() => {
        ...
        if (authUser?.role === 'CUSTOM' && authUser?.customRole) {
            const permissions = authUser.customRole.permissions || [];
            if (permissions.includes('sixteen_day:edit_layout')) return true;
            
            const pagePermission = authUser.customRole.allowedPages?.find(p => p.key === '16-day-monitoring');
    ...
    return (
        <div className="space-y-6 w-full max-w-none mx-auto pb-20 p-4 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500 font-medium">Daily monitoring and progress tracking</p>
                    </div>
                </div>
            </div>
                                                    ...
                                                    {searchedOperators.length > 0 ? searchedOperators.map((s) => (
                                                        <TableRow 
                                                            ...
                                                            className={cn(
                                                                ...
                                                                studentId === (s._id || s.id) ? "bg-indigo-50/80" : "hover:bg-slate-50/50"
                                                            )}
                                                        >
            ...
            {!isEmployee && dept && !studentId && (
                <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-3 flex items-start gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
                    <IconInfoCircle className="w-5 h-5 text-indigo-500 mt-0.5 shrink-0" />
                    <div>
                        <h4 className="text-sm font-bold text-indigo-800">Assign Selection</h4>
                        <p className="text-xs text-indigo-700/80 leading-relaxed max-w-2xl">
                            Please select an operator from the dropdown hierarchy above to view or track their 16-day monitoring progress.
                        </p>
                    </div>
                </div>
            )}
            ...
            {dept ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-200">
                    ...
                    <p className="text-sm text-slate-500 max-w-xs text-center mt-2 leading-relaxed">
                        Choose a department to view and manage its 16-day monitoring check-sheet configuration.
                    </p>
                </div>
            )}
        </div>
