...
const QuizMonitoring = () => {
  ...
  const { data: sectionsData } = useGetSectionsByDepartmentQuery(deptQueryId, { skip: !deptQueryId });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Monitor and audit all submitted test papers across your production hierarchy (Read-only View)
          </p>
        </div>
                      ...
                      <TableRow key={attempt._id || attempt.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <TableCell className="text-center pr-6 py-4">
                          <Button 
                            size="sm" 
                            variant="outline" 
                            onClick={() => navigate(`/trainer/quiz-monitoring/review/${attempt._id || attempt.id}`)}
