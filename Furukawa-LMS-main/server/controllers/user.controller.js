...
/**
 * Get Multi-Skilling Students (custom endpoint returning all matching users in department/section without page limits or temporary filters)
 */
export const getMultiSkillingStudents = asyncHandler(async (req, res) => {
  let whereClauses = [
    "((u.isEmployee = 1) OR (u.role = 'CUSTOM' AND (u.isTrainer = 0 OR u.isTrainer IS NULL)))",
    "(u.isTrainer = 0 OR u.isTrainer IS NULL)",
    "(u.isDeleted = 0 OR u.isDeleted IS NULL)",
    "(u.status IS NULL OR u.status != 'LEFT')"
  ];
  ...
  }, "Multi-skilling students fetched successfully"));
});
