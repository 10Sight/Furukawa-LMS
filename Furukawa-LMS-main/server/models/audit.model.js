    ...
    static async init() {
        ...
        try {
            ...
            try {
                ...
                // This includes both strict and filtered indexes to avoid dependency errors during ALTER COLUMN
                ...
                for (const row of idxRows) {
                    try {
                        await executeQuery(`DROP INDEX ${row.name} ON users`);
                         ...
                         try { await executeQuery(`ALTER TABLE users DROP CONSTRAINT ${row.name}`); } catch(e2) {}
                    }
                }
