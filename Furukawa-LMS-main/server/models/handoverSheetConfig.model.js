...
class HandoverSheetConfig {
    ...
    static async init() {
        ...
        const historyTableQuery = `
            ...
            BEGIN
                ...
                CREATE INDEX idx_hs_dept_history ON handover_sheet_config_history(departmentId)
            END
        `;
    ...
    static async findByDepartmentId(departmentId) {
        const [rows] = await executeQuery("SELECT * FROM handover_sheet_configs WHERE departmentId = ?", [departmentId]);
    ...
    static async upsert(departmentId, configData, remark = "Layout updated", updatedBy = "System") {
        ...
        const historyQuery = `
            INSERT INTO handover_sheet_config_history (departmentId, config, remark, updatedBy, createdAt)
            VALUES (?, ?, ?, ?, GETDATE())
        `;
    ...
    static async getHistory(departmentId, limit = 20) {
        const query = "SELECT TOP (?) * FROM handover_sheet_config_history WHERE departmentId = ? ORDER BY createdAt DESC";
