...
class SkillMatrixDashboardConfig {
    ...
    static async findByDepartmentId(departmentId) {
        const [rows] = await executeQuery("SELECT * FROM skill_matrix_dashboard_configs WHERE departmentId = ?", [departmentId]);
        if (rows.length === 0) return null;
        return new SkillMatrixDashboardConfig(rows[0]);
    }
    ...
    static async upsert(departmentId, configData, remark = "Layout updated", updatedBy = "System") {
        ...
        const query = `
            MERGE INTO skill_matrix_dashboard_configs AS target
        ...
        const historyQuery = `
            INSERT INTO skill_matrix_dashboard_config_history (departmentId, config, remark, updatedBy, createdAt)
            VALUES (?, ?, ?, ?, GETDATE())
        `;
    ...
    static async getHistory(departmentId, limit = 20) {
        const query = "SELECT TOP (?) * FROM skill_matrix_dashboard_config_history WHERE departmentId = ? ORDER BY createdAt DESC";
...
export default SkillMatrixDashboardConfig;
