...
class SkillUpgradationPlanConfig {
    ...
    static async findByDepartmentId(departmentId) {
        const [rows] = await executeQuery("SELECT * FROM skill_upgradation_plan_configs WHERE departmentId = ?", [departmentId]);
        if (rows.length === 0) return null;
        return new SkillUpgradationPlanConfig(rows[0]);
    }
    ...
    static async upsert(departmentId, configData, remark = "Layout updated", updatedBy = "System") {
        ...
        const query = `
            MERGE INTO skill_upgradation_plan_configs AS target
        ...
        const historyQuery = `
            INSERT INTO skill_upgradation_plan_config_history (departmentId, config, remark, updatedBy, createdAt)
            VALUES (?, ?, ?, ?, GETDATE())
        `;
    ...
    static async getHistory(departmentId, limit = 20) {
        const query = "SELECT TOP (?) * FROM skill_upgradation_plan_config_history WHERE departmentId = ? ORDER BY createdAt DESC";
...
// Initialize tables
SkillUpgradationPlanConfig.init().catch(err => console.error("Failed to initialize SkillUpgradationPlanConfig tables:", err));

export default SkillUpgradationPlanConfig;
