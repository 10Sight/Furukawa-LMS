import { Router } from "express";
import {
    createCourse,
    getCourses,
    getCourseById,
    updatedCourse,
    deleteCourse,
    getCourseAnalytics,
    getCourseStudents,
    getSoftDeletedCourses,
    restoreCourse
} from "../controllers/course.controller.js";
import verifyJWT from "../middlewares/auth.middleware.js";
import authorizeRoles from "../middlewares/authrization.middleware.js";

const router = Router();

router.post("/", verifyJWT, authorizeRoles("isTrainer", "isAdmin", "course:create"), createCourse);

// Super admin specific routes - must come before /:id routes
router.get("/deleted/all", verifyJWT, authorizeRoles("SUPERADMIN"), getSoftDeletedCourses);
router.patch("/deleted/:id/restore", verifyJWT, authorizeRoles("SUPERADMIN"), restoreCourse);

router.get("/", verifyJWT, authorizeRoles("isTrainer", "isAdmin", "isEmployee", "SUPERADMIN", "course:read"), getCourses);
router.get("/:id", verifyJWT, authorizeRoles("isTrainer", "isAdmin", "isEmployee", "course:read"), getCourseById);
router.get("/:id/analytics", verifyJWT, authorizeRoles("isTrainer", "isAdmin", "analytics:read"), getCourseAnalytics);
router.get("/:id/students", verifyJWT, authorizeRoles("isTrainer", "isAdmin", "course:update"), getCourseStudents);
router.put("/:id", verifyJWT, authorizeRoles("isTrainer", "isAdmin", "course:update"), updatedCourse);
router.delete("/:id", verifyJWT, authorizeRoles("isTrainer", "isAdmin", "SUPERADMIN", "course:delete"), deleteCourse);

export default router;
